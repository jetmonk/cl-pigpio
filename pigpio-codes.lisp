...
(defpackage pigpio-codes
  ...
  (:export
   ...
   #:+pi_input+ #:+pi_output+
   #:+pi_alt0+ #:+pi_alt1+ #:+pi_alt2+
   #:+pi_alt3+ #:+pi_alt4+ #:+pi_alt5+
   #:+pi_pud_off+ #:+pi_pud_down+ #:+pi_pud_up+
   
   ))

  

(in-package pigpio-codes)
...
(defparameter  *pigpio-errorcodes* ;; from pgpio.h
...
;; for an error number, return (values error-name error-desc)
(defun pigpio-error-code-name (ncode)
  "Given NCODE, an pigpio errorcode, return (VALUES ERROR-NAME ERROR-DESC)"
  (let ((err-list (find ncode *pigpio-errorcodes* :key 'second)))
    (if err-list
	(values
	 (first err-list)
	 (or (third err-list) "No description available."))
	(values
	 "UNKNOWN-ERROR"
	 "UNKNOWN-ERROR"))))
...
;; for a pigpio function that returns an integer, handle the error code
;; BODY is expected to be a call to a pigpio FFI function
(defmacro handle-pigpio-error (&Body body)
  "Macro that catches pigpio errors: given a BODY that can return a
negative PIGPIO error code, throws a descriptive error if such an
error code is returned."
  `(let ((%retval ,@body))
     (when (minusp %retval)
       (multiple-value-bind (%err-name %err-desc)
	   (pigpio-error-code-name %retval)
	 (error "Pigpio error ~A (~A): ~A" %err-name %retval %err-desc)))
     %retval))
